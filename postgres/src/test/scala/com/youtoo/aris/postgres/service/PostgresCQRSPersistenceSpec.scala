...
import com.youtoo.aris.postgres.*
import com.youtoo.aris.postgres.config.*
...
object PostgresCQRSPersistenceSpec extends PgSpec, TestSupport {
  ...
  def spec =
    suite("PostgresCQRSPersistenceSpec")(
        ...
        check(namespaceGen, keyGen, versionGen, keyGen, keyGen, discriminatorGen) {
          (ns, key, version, grandParentId, parentId, discriminator) =>
            ...
            for {
              ...
              tx <- ZIO.service[TransactionManager[ZConnection]]
      ...
      ZLayer.makeSome[ZConnectionPool, CQRSPersistence](
        ZIOJdbcTransactionManager.live() >>> PostgresCQRSPersistence.live(),
      ),
