...
case class FetchOptions(offset: Option[Key], limit: Option[Int], order: FetchOptions.Order)

object FetchOptions {
  inline def apply(): FetchOptions = FetchOptions(None, None, Order.asc)
  inline def apply(offset: Key, limit: Int, order: FetchOptions.Order = Order.asc): FetchOptions =
  ...
  inline def limit(value: Int): FetchOptions = FetchOptions(offset = None, limit = value.some, order = Order.asc)
  ...
  extension (options: FetchOptions) inline def limit(value: Int): FetchOptions = options.copy(limit = Some(value))
