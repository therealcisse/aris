...
object MailConfigEventStoreSpec extends MockSpecDefault, TestSupport {

  def spec = suite("MailConfigEventStoreSpec")(
    testReadEventsId,
  ...
  val discriminator = MailConfigEvent.discriminator

  val testReadEventsId = test("readEvents by Id") {
    check(keyGen, Gen.listOf(mailConfigEventChangeGen)) { (id, events) =>
      val mockEnv = MockCQRSPersistence.ReadEvents.Full.of(
        equalTo((id, discriminator, MailConfigEventStore.Table)),
        value(events.toChunk),
      )

      val effect = for {
        store <- ZIO.service[MailConfigEventStore]
        result <- store.readEvents(id).atomically
      } yield assert(result)(equalTo(NonEmptyList.fromIterableOption(events)))

      effect.provideSomeLayer[ZConnectionPool & Tracing](mockEnv.toLayer >>> MailConfigEventStore.live())
    }
  }

  val testReadEventsByIdAndVersion = test("readEvents by Id and Version") {
    check(keyGen, versionGen, Gen.listOf(mailConfigEventChangeGen)) { (id, version, events) =>
      val mockEnv = MockCQRSPersistence.ReadEvents.Snapshot.of(
        equalTo((id, discriminator, version, MailConfigEventStore.Table)),
        value(events.toChunk),
      )
  ...
  val testReadEventsQueryOptions = test("readEvents by Query and Options") {
    check(Gen.listOf(mailConfigEventChangeGen)) { events =>
      val query = PersistenceQuery.condition()
      val options = FetchOptions()

      val mockEnv = MockCQRSPersistence.ReadEvents.FullArgs.of(
        equalTo((discriminator, query, options, MailConfigEventStore.Table)),
        value(events.toChunk),
      )
  ...
  val testReadEventsQueryOptionsAggregate = test("readEvents by Query and Options aggregate") {
    check(keyGen, Gen.listOf(mailConfigEventChangeGen)) { (id, events) =>
      ...
      val mockEnv = MockCQRSPersistence.ReadEvents.FullArgsByAggregate.of(
        equalTo((id, discriminator, query, options, MailConfigEventStore.Table)),
        value(events.toChunk),
      )
  ...
  val testSaveEvent = test("save an event") {
    check(keyGen, versionGen, mailConfigEventGen) { (id, version, event) =>
      val change = Change(version, event)
      val returnId = 1L

      val mockEnv = MockCQRSPersistence.SaveEvent.of[(Key, Discriminator, Change[MailConfigEvent], Catalog), Long](
        equalTo((id, discriminator, change, MailConfigEventStore.Table)),
        value(returnId),
      )
