...
//
//   type Environment =
//     Migration & ZConnectionPool & CQRSPersistence & SnapshotStore & MigrationEventStore & MigrationCQRS & MigrationProvider & MigrationCheckpointer & Server & Server.Config & NettyConfig & MigrationService & MigrationRepository & PrometheusPublisher & MetricsConfig & SnapshotStrategy.Factory
//
...
//         case Some(ingestion) =>
//           val bytes = summon[BinaryCodec[Migration]].encode(ingestion)
//
//           Response(
//             Status.Ok,
//             Headers(Header.ContentType(MediaType.application.json).untyped),
...
//       val key = Key.wrap(id)
//
//       req.body.fromBody[MigrationCommand] foldZIO (
//         failure = _ => ZIO.succeed(Response.notFound),
//         success = cmd => MigrationCQRS.add(key, cmd) `as` Response.ok
//       )
//
//     },
...
//         case Right(n) =>
//           MigrationCQRS.load(id = Key.wrap(id)) map {
//             case None => Response.notFound
//             case Some(ingestion) =>
...
//
//         _ <- MigrationCQRS.add(id, MigrationCommand.StartMigration(Migration.Id(id), timestamp))
//
//         opt <- MigrationCQRS.load(id)
//
//         _ <- opt.fold(ZIO.unit) { ingestion =>
//           atomically {
//             MigrationService.save(ingestion)
//           }
//         }
//
